...
import { SummaryCard, AreaChart, PieChart, StatsCard, ScatterChart, LineChart, BoxPlotChart } from '@/components/Statistics'
...
const FocusStatsPage: React.FC = () => {
  ...
  const stats = useMemo(() => {
    ...
    // 每日专注总时长（分钟）- 用于折线图
    const dailyDurationData: { date: string; value: number }[] = daysData.map(d => ({ ...d }))

    // 每日专注时长分布数据（分钟）- 用于箱型图
    const dailyDurationValues: number[] = daysData.filter(d => d.value > 0).map(d => d.value)

    // 各时段近N天每日专注次数趋势（面积图）
    const periods = ['早晨(6-9)', '上午(9-12)', '下午(12-18)', '晚上(18-24)', '深夜(0-6)']
    const periodTrendData: { date: string; value: number }[] = []
    for (let i = dateRange - 1; i >= 0; i--) {
      const d = new Date()
      d.setDate(d.getDate() - i)
      d.setHours(0, 0, 0, 0)
      ...
      periodTrendData.push({
        date: `${d.getMonth() + 1}/${d.getDate()}`,
        value: count,
      })
    }

    return { totalSessions, totalHours, avgMinutes, daysData, periodPieData, dailyCountData, scatterData, dailyDurationData, dailyDurationValues, periodTrendData }
      ...
      <div className="max-w-4xl mx-auto">
        ...
        {focusOnly.length === 0 ? (
          ...
          <div className="space-y-4">
            ...
            {/* 折线图：近N天每日专注总时长趋势 */}
            <LineChart title={`近${dateRange}天每日专注总时长趋势（分钟）`} data={stats.dailyDurationData} color="#6B4C9A" />

            {/* 箱型图：每日专注时长分布 */}
            <BoxPlotChart title="每日专注时长分布（分钟）" data={stats.dailyDurationValues} />

            {/* 面积图：各时段专注次数趋势 */}
            <AreaChart title={`近${dateRange}天各时段专注次数趋势`} data={stats.periodTrendData} color="#9B7EC9" />
          </div>
        )}
      </div>
    </div>
