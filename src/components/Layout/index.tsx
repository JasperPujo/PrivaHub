...
const Layout: React.FC<LayoutProps> = ({ children, className }) => {
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      // 同步 walls（主题墙）
      const { syncPull, syncPush, fullSync, noteWallToDb, noteWallFromDb, taskToDb, taskFromDb, scheduleToDb, scheduleFromDb, planToDb, planFromDb, habitToDb, habitFromDb, noteToDb, noteFromDb, trackerCategoryToDb, trackerEntryToDb } = await import('@/lib/sync')
      const wallsPull = await syncPull('note_walls', user.id, noteWallFromDb)
      if (wallsPull.success && wallsPull.data.length > 0) noteStore.setWalls(wallsPull.data)
      const wallsPush = await syncPush({ table: 'note_walls', userId: user.id, data: noteStore.walls, toDbRow: noteWallToDb })

      const results = await fullSync(user.id, {
        tasks: { table: 'tasks', getData: () => todoStore.tasks, setData: (data) => todoStore.setTasks(data), toDbRow: taskToDb, fromDbRow: taskFromDb },
        schedules: { table: 'schedules', getData: () => scheduleStore.schedules, setData: (data) => scheduleStore.setSchedules(data), toDbRow: scheduleToDb, fromDbRow: scheduleFromDb },
        plans: { table: 'plans', getData: () => planStore.plans, setData: (data) => planStore.setPlans(data), toDbRow: planToDb, fromDbRow: planFromDb },
        habits: { table: 'habits', getData: () => habitStore.habits, setData: (data) => habitStore.setHabits(data), toDbRow: habitToDb, fromDbRow: habitFromDb },
        notes: { table: 'notes', getData: () => noteStore.notes, setData: (data) => noteStore.setNotes(data), toDbRow: noteToDb, fromDbRow: noteFromDb },
        trackerCategories: { table: 'tracker_categories', getData: () => trackerStore.categories, setData: (data) => trackerStore.setCategories(data), toDbRow: trackerCategoryToDb },
        trackerEntries: { table: 'tracker_entries', getData: () => trackerStore.entries, setData: (data) => trackerStore.setEntries(data), toDbRow: trackerEntryToDb },
      })

      const anyError = Object.values(results).some((r: any) => !r.push.success || !r.pull.success)
      if (anyError) {
        addNotification({ message: '部分数据同步失败，请检查控制台', type: 'warning' })
      } else {
